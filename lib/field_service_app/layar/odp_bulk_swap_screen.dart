...
import 'package:field_technician_app/field_service_app/data/odp_bulk_swap_model.dart';
...
class _OdpBulkSwapScreenState extends State<OdpBulkSwapScreen> {
  ...
  Widget _buildPortColumn(
      OdpDevice device, Map<String, GlobalKey> keys, bool isOldDevice) {
    return Expanded(
  ...
  Widget _buildPortItem(
      Port port, GlobalKey key, bool isOldDevice, bool isOverride) {
        ...
        children: [
          if (!isOldDevice)
            CircleAvatar(
                radius: 4,
                backgroundColor:
                    isOverride ? const Color(0xFF137fec) : Colors.grey),
          Text(port.id,
...
class ConnectionPainter extends CustomPainter {
  ...
  ConnectionPainter(
      {required this.ports, required this.oldKeys, required this.newKeys});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var port in ports) {
      if (port.mappedToPortId != null) {
        ...
        if (oldKey != null &&
            newKey != null &&
            oldKey.currentContext != null &&
            newKey.currentContext != null) {
          ...
          final startPos = oldBox
              .localToGlobal(Offset(oldBox.size.width, oldBox.size.height / 2));
          final endPos =
              newBox.localToGlobal(Offset(0, newBox.size.height / 2));

          final path = Path();
          path.moveTo(startPos.dx, startPos.dy);
          path.cubicTo(startPos.dx + 40, startPos.dy, endPos.dx - 40, endPos.dy,
              endPos.dx, endPos.dy);

          final isOverride = port.id != port.mappedToPortId;
          if (isOverride) {
            overridePaint.shader = const LinearGradient(
