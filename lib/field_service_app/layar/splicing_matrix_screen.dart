...
import 'package:field_technician_app/field_service_app/workflow/splicing_matrix_workflow.dart';
...
class _SplicingMatrixScreenState extends State<SplicingMatrixScreen> {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Row(
            children: [
              _buildCableColumn(_workflow.cableA!, surfaceColor,
                  CrossAxisAlignment.start, _keysA, true),
              _buildCableColumn(_workflow.cableB!, surfaceColor,
                  CrossAxisAlignment.end, _keysB, false),
            ],
          ),
          CustomPaint(
  ...
  Widget _buildCableColumn(FiberCable cable, Color surfaceColor,
      CrossAxisAlignment alignment, Map<int, GlobalKey> keys, bool isLeft) {
    return Expanded(
      child: Column(
              ...
              itemBuilder: (context, index) {
                ...
                return Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                          'Tube ${tube.id} - ${tube.color.toString().split('.').last}',
                          style: const TextStyle(fontWeight: FontWeight.bold)),
                      const SizedBox(height: 8),
                      ...tube.cores
                          .map((core) => _buildCoreItem(
                              core, surfaceColor, keys[core.id]!, isLeft))
                          .toList(),
                    ],
                  ),
                );
  ...
  Widget _buildCoreItem(
      FiberCore core, Color surfaceColor, GlobalKey key, bool isLeft) {
    return Container(
      key: key,
        ...
        children: [
          if (isLeft) ...[
            Container(
                ...
                height: 12,
                decoration: BoxDecoration(
                    color: getFiberColor(core.color), shape: BoxShape.circle)),
...
class ConnectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var connection in connections) {
      ...
      if (keyA != null &&
          keyB != null &&
          keyA.currentContext != null &&
          keyB.currentContext != null) {
        ...
        final posA = renderBoxA.localToGlobal(
            Offset(renderBoxA.size.width, renderBoxA.size.height / 2));
        final posB =
            renderBoxB.localToGlobal(Offset(0, renderBoxB.size.height / 2));

        final path = Path();
        path.moveTo(posA.dx, posA.dy);
        path.cubicTo(
            posA.dx + 100, posA.dy, posB.dx - 100, posB.dy, posB.dx, posB.dy);

        // Use the color of the source fiber
        final core = cableA.tubes
            .expand((tube) => tube.cores)
            .firstWhere((core) => core.id == connection.fromCoreId);
